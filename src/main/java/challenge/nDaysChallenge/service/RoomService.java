...
public class RoomService {
     ...
     * 전체 완료 챌린지 조회
     */
    public List<RoomResponseDto> findFinishedSingleRooms(Member member) {
        List<Room> rooms = Stream.concat(
                singleRoomRepository.finishedSingleRooms(member).stream(),
                groupRoomRepository.finishedGroupRoom(member).stream())
    ...
    private void checkRoomAndMember(Member member, Room room) {
        Member findMember = singleRoomRepository.findMemberByRoomNumber(room.getNumber()).orElseThrow(
